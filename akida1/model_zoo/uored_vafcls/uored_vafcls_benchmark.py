...
Runs a per-layer timing benchmark on an Akida UORED-VAFCLS model, prints a
summary table, and generates summary plots.

The numbers describe one model trained on one fold. They are a property of the
architecture on this hardware, not of the model's diagnostic performance - for
that see uored_vafcls_cross_validate.py.

Example
-------
    python uored_vafcls_benchmark.py -l models/akdcnn_uored_vafcls_qat.fbz
"""
import argparse
import json
import pathlib
...
from uored_vafcls_data import FIXED_FOLD, get_samples

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Per-layer hardware benchmark for an Akida UORED-VAFCLS model')
    ...
    parser.add_argument('--fold', type=int, default=FIXED_FOLD,
                        help='Fold whose training recordings the benchmark '
                             'windows are drawn from')
    parser.add_argument('--seed', type=int, default=0,
                        help='Random seed for the benchmark sample draw')
    parser.add_argument('--save-metrics', action='store_true',
                        help='Write benchmark values to metrics.json. Only '
    ...
    if args.save_metrics and args.fold != FIXED_FOLD:
        sys.exit(f'--save-metrics writes the README hardware table, which is '
                 f'fold {FIXED_FOLD}; refusing to write fold {args.fold}.')
    ...
    for mm in map_modes:
        ...
        if full_results[mm] is None:
            sys.exit(f'The model did not map fully to hardware in {mm} mode; '
                     f'benchmarking cannot proceed.')
