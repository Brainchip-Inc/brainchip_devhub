...
"""
Float-model activation sparsity measurement for VWW.

vww_sparsity_only.py measures sparsity via akida_models.sparsity.compute_sparsity,
which requires a quantized/Akida model and runs inference through a software
backend (see SPARSITY_EXPERIMENT.md's note on why those numbers aren't directly
comparable to hardware-measured sparsity). The joint activation+structured sparsity
experiment stays float-only end to end, so this instead measures the same "mean
fraction of zero activations across ReLU layers" statistic directly on a float
tf_keras model's ReLU outputs -- no quantization/conversion involved.

Example
-------
    python vww_float_sparsity.py -l models/akidanet_vww.h5 -d ./data/vw_coco2014_96
"""
import argparse
