...
"""
Structured (channel/filter) sparsity for VWW via Network Slimming
...
`vww_model.py`'s AkidaNet-alpha0.25 is a strict feed-forward chain (no
...
def prune_model(model, target_fraction, prune_layer_names):
    ...
    for layer in model.layers[1:]:  # skip the original InputLayer
        ...
        # Everything else (Conv2D stem, ReLU, pooling, input scaling, Dense head, ...)
        # is channel-count-preserving as long as no upstream pruning is pending
        # (current_keep_idx is None) by the time we reach it -- true here since
        # every prune-eligible layer's downstream neighbor consumes the pending
        # index immediately (the next SeparableConv2D or BN handles it above).
