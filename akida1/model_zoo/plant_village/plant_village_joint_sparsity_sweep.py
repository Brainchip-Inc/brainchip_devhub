...
"""
Joint activation + structured sparsity sweep for PlantVillage (AkidaNet
alpha=0.5), mirroring ../vww/vww_joint_sparsity_sweep.py:
  - activation sparsity: HoyerSquare(normalize=True) activity regularization on
    every ReLU layer (see SPARSITY_EXPERIMENT.md -- reg=36 is the established
    best point on this axis alone, 78.2% Akida-measured sparsity for a ~1.5pt
    accuracy cost).
  - structured sparsity: Network Slimming channel pruning of separable_4..
    separable_12's output channels (see plant_village_structured_sparsity.py).
...
JOINT_SPARSITY_EXPERIMENT.md for why. Per grid point:
  ...
  2. Phase 1 float train, with the activation regularizer attached if reg > 0
     (train_plant_village, imported from plant_village_train.py)
  ...
  4. Phase 2 float fine-tune (skipped if prune_target == 0), re-attaching the
     ...
     don't carry it over
  5. measure float accuracy (on the held-out test split, matching
     plant_village_eval.py's own convention), float activation sparsity, and
     parameter count

Example
-------
    python plant_village_joint_sparsity_sweep.py --data ./data/plant_village

    # Follow-up: add lighter points to an existing summary CSV without re-running it
    python plant_village_joint_sparsity_sweep.py --data ./data/plant_village \\
        --points activation_light:15:0 structured_light:0:0.2 joint_light:15:0.2
"""
import argparse
import csv
...
from plant_village_data import get_data, get_samples
from plant_village_train import train_plant_village
from plant_village_structured_sparsity import add_gamma_l1_loss, prune_model
from plant_village_float_sparsity import compute_float_sparsity

PRUNABLE_LAYERS = [f'separable_{i}' for i in range(4, 13)]
REG_TYPE = 'hoyer_square_norm'


def run_point(reg, prune_target, args, tag_dir, train_ds, val_ds, test_ds):
    ...
    # Phase 1: float train
    train_plant_village(model, train_ds, val_ds, epochs=args.phase1_epochs,
    ...
    if prune_target > 0:
        ...
        train_plant_village(final_model, train_ds, val_ds, epochs=args.phase2_epochs,
                             learning_rate=args.phase2_lr, seed=args.seed, **reg_kwargs)

    final_model.compile(metrics=['accuracy'])
    # Evaluated on the held-out test split, matching plant_village_eval.py's
    # own "Test accuracy" convention (as opposed to VWW, which evaluates on
    # its validation split).
    _, accuracy = final_model.evaluate(test_ds, verbose=0)
...
def main():
    ...
    p.add_argument('--data', default='./data/plant_village',
                    help='PlantVillage tfds data directory')
    p.add_argument('--reg', type=float, default=36,
    ...
    p.add_argument('--gamma-l1-strength', type=float, default=1e-5,
                    help='L1 strength on BN gamma during Phase 1, when prune_target > 0 '
                         '(starting guess, reused from the VWW experiment -- expect to '
                         'revisit given this model\'s larger per-layer size imbalance)')
    p.add_argument('--phase1-epochs', type=int, default=10,
                    help='Matches plant_village_train.sh\'s float-training default')
    ...
    p.add_argument('--points', nargs='+', default=None,
                    help='Explicit follow-up grid points as "tag:reg:prune_target" (e.g. '
                         '"joint_light:15:0.2"), run in addition to whatever is already in '
    ...
    rows = []
    if args.points:
        # Follow-up sweep: keep whatever's already in the CSV (e.g. the initial
    ...
    train_ds, val_ds, test_ds = get_data(args.data, (224, 224, 3), args.batch_size,
                                          seed=args.seed)

    for tag, reg, prune_target in grid:
        print(f"\n==== {tag} (reg={reg:g}, prune_target={prune_target:g}) ====\n")
        row = run_point(reg, prune_target, args, out_dir / tag, train_ds, val_ds, test_ds)
        row['tag'] = tag
        rows.append(row)
