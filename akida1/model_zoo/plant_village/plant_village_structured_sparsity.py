...
"""
Structured (channel/filter) sparsity for PlantVillage via Network Slimming
(Liu et al. 2017, "Learning Efficient Convolutional Networks through
Network Slimming"): an L1 penalty on each targeted BatchNormalization
layer's gamma (per-channel scale) during training identifies which output
channels of the preceding SeparableConv2D contribute least, then those
channels are physically pruned and the surrounding layers resized to match.

Unlike activation-sparsity regularization (SPARSITY_EXPERIMENT.md), this
axis reduces the model's actual parameter count -- Akida 1 hardware only
...
names), so it applies unchanged to PlantVillage's AkidaNet-alpha0.5 backbone.

`plant_village_model.py`'s backbone is a strict feed-forward chain (no
residual/skip connections) identical in structure to VWW's, confirmed via
`model.summary()`, so pruning a SeparableConv2D's output channels only ever
affects the very next weighted layer's input dimension. PlantVillage adds an
extra `fc_1` Dense block (with its own BN+ReLU) between the backbone's last
SeparableConv2D (`separable_13`) and the `predictions` head, which is exactly
why -- as with VWW -- `separable_13` is excluded from the prunable set (see
`plant_village_joint_sparsity_sweep.py`).
"""
import numpy as np
...
def prune_model(model, target_fraction, prune_layer_names):
    ...
    for layer in model.layers[1:]:  # skip the original InputLayer
        ...
        # Everything else (Conv2D stem, ReLU, pooling, input scaling, the fc_1/
        # predictions Dense head, ...) is channel-count-preserving as long as no
        # upstream pruning is pending (current_keep_idx is None) by the time we
        # reach it -- true here since every prune-eligible layer's downstream
        # neighbor consumes the pending index immediately (the next
        # SeparableConv2D or BN handles it above), and separable_13 (the last
        # backbone layer, never itself pruned) resets current_keep_idx to None
        # before the fc_1 Dense block is reached.
