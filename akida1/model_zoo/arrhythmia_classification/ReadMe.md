...
The MIT-BIH Arrhythmia Database was partitioned using the standard inter-patient split protocol.
...
To create a validation set for model selection and performance monitoring, the training dataset was further divided using an **80:20 stratified split**:
...
- Training set: **80%** of the samples from the training records
- Validation set: **20%** of the samples from the training records
- Stratification: Preserves the class distribution of Normal (N), Supraventricular (S), and Ventricular (V) beats across both subsets
- Random state: **42** (ensures reproducible train/validation partitioning)

### Summary

| Dataset | Source |
|----------|---------|
| Training | 22 predefined MIT-BIH training records |
| Validation | 20% stratified split from training samples |
| Testing | 22 predefined MIT-BIH test records |
