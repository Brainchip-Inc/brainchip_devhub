...
from data import ECGDatasetBuilder
...
DEFAULT_SPLITS   = "./processed_data"

def process_single_beat(ecg, peaks, peak, symbol, i, builder):
    """Isolated processing framework frame for a single heartbeat to run inside Parallel worker thread."""
    if symbol not in LABEL_MAP:
        return None

    t0 = time.perf_counter()
    beat = builder.extract_beat(ecg, peak)
    ...
    t0 = time.perf_counter()
    scalogram = builder.beat_to_scalogram(beat)
    t_scalogram = time.perf_counter() - t0

    t0 = time.perf_counter()
    rr_feat = builder.compute_rr_features(peaks, i)
...
def load_records(records: list, dataset_path: str, builder):
    X, RR, y = [], [], []
    ...
    for record in records:
        ...
        results = Parallel(n_jobs=-1, backend="loky")(
            delayed(process_single_beat)(ecg, peaks, peak, symbol, i, builder)
...
def build_dataset(dataset_path: str, splits_dir: str):
    print("\n[1/3] Loading TEST records (DS2) with Parallel Processing ...")
    builder = ECGDatasetBuilder(dataset_path, data_dir=splits_dir, window_before=WINDOW_BEFORE, window_after=WINDOW_AFTER, img_size=IMG_SIZE)

    X_test, RR_test, y_test = load_records(TEST_RECORDS, dataset_path, builder)
    ...
    X_test  = builder.add_rr_rows(X_test,   RR_test_n)
...
def save_splits(splits_dir, X_test, y_test):
...
def main():
    ...
    if args.load_splits:
        X_test, y_test = load_splits(args.splits_dir)
    else:
        X_test, y_test = build_dataset(args.dataset_path,args.splits_dir)
        if args.save_splits:
            print("\nSaving splits ...")
            save_splits(args.splits_dir, X_test, y_test)
