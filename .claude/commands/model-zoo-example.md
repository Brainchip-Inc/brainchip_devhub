...
### 3i-2. `docs/zoo_card.json`

One row in the Akida 1 landing README's model zoo table (`akida1/README.md`, generated by
`akida1/update_readme.py`). The card holds only fixed text plus pointers into `metrics.json`,
so the table updates itself when the metrics change. Copy `akida1/model_zoo/vww/docs/zoo_card.json`
and edit it:
