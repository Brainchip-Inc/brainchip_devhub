"""Colab setup for the PlantVillage (Akida 2) training notebook.
...
(see plant_village_data.py, plant_village_model.py, plant_village_train.py for that).
...
If you ARE on Colab, this is the file that gets you running:
  ...
  - Points Python at the right folders (repo root for brainchip_utils,
    this example's folder for plant_village_data / plant_village_model /
    plant_village_train)
  - Installs akida_models, tf_keras, quantizeml

The PlantVillage dataset is NOT downloaded here: it is fetched automatically by
TensorFlow Datasets the first time the data pipeline runs (see
plant_village_data.py), so there is nothing to pre-download.
...
EXAMPLE_SUBDIR = 'akida2/model_zoo/plant_village'
...
def setup():
    ...
    # The PlantVillage dataset is downloaded on demand by tensorflow_datasets
