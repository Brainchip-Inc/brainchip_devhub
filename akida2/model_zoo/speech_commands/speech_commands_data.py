...
def get_data(data_path, batch_size=100, data_transform=None, seed=42, **kwargs):
    """Load the Speech Commands dataset as batched tf.data.Datasets.

    Returns (train_ds, test_ds, val_ds), each yielding (mfcc_features, label).
    When data_transform is provided, features are uint8; otherwise float32.
    Thin wrapper over get_datasets().
    """
    return get_datasets(
        data_dir=data_path,
