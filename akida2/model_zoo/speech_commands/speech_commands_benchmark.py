...
"""
Speech Commands Keyword Spotting (KWS) hardware benchmark for Akida 2.

Runs a latency benchmark on an Akida Speech Commands model, prints a summary, checks
per-layer activation sparsity, and generates summary plots.

Benchmark details:
  * The Akida 2 reference hardware is an FPGA running at 25 MHz, so the
    measured clock frequency is 25e6.
...
Example
-------
    python speech_commands_benchmark.py -l models/ds_cnn_speech_commands_i8_w8_a8.fbz
"""
import argparse
import json
import pathlib
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Hardware latency benchmark for an Akida 2 KWS model')
    ...
    # split, matching the model's uint8 input expectation.
    ...
    if args.save_metrics:
        ...
        #
        # Keys are variant-prefixed to match the multi-variant README table.
        # The variant is inferred from the loaded .fbz filename (same scheme as
        # speech_commands_eval.py). Power keys are intentionally absent:
        # benchmarking is latency-only until the FPGA power path exists.
